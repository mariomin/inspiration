...
@AtsStrategy(chineseName = "日内反转策略", param = ReverseStrategyParam.class)
public class ReverseStrategy extends Strategy<ReverseStrategyParam, ReverseAnalysisResult> {
	private final static Logger log = LoggerFactory.getLogger(ReverseStrategy.class);
	private final ReverseDataCache dataCache = new ReverseDataCache();
	...
	public void subscribe() {
		...
		symbols.add(params.getIndexCode());
	...
	public void createSignalAnalyzer() {
		signalAnalyzer = new ReverseSignalAnalyzer(dataCache);
	}
	
	@Override
	public void createTradeExecutor() {
		//tradeExecutor = new ReverseTradeExecutor(dataCache, subscribeService);
		tradeExecutor = new SimIntradayTradeExecutor(dataCache, subscribeService);
	}
	
	@Override
	public void onMarketData(MarketData marketData) {
		if ((getState() != StrategyState.Initializing) && (getState() != StrategyState.Stopped)) {
			ReverseAnalysisResult result = signalAnalyzer.analyze(marketData);
