 ...
 */
public class ATSDefaultMessageFactory implements MessageFactory {
	private final Map<String, MessageFactory> messageFactories = new ConcurrentHashMap<String, MessageFactory>();
	
	/**
	 * Constructs a DefaultMessageFactory, which dynamically loads and delegates
	 * to the default version-specific message factories, if they are available
	 * at runtime.
	 * <p>
	 * Callers can set the {@link Thread#setContextClassLoader context
	 * classloader}, which will be used to load the classes if
	 * {@link Class#forName Class.forName} fails to do so (e.g. in an OSGi
	 * environment).
	 */
	public ATSDefaultMessageFactory() {
	...
	/**
	 * Adds a factory of the given class, which will be delegated to for
	 * creating Message instances from messages with the given begin string.
	 * <p>
	 * Callers can set the {@link Thread#setContextClassLoader context
	 * classloader}, which will be used to load the classes if
	 * {@link Class#forName Class.forName} fails to do so (e.g. in an OSGi
	 ...
	 *
	 * @param beginString
	 *            the begin string whose messages will be delegated to the
	 *            factory
	 * @param factoryClassName
	 *            the name of the factory class to instantiate and add
	 * @throws ClassNotFoundException
	 *             if the named factory class cannot be found
	 * @throws RuntimeException
	 *             if the named factory class cannot be instantiated
	 */
	 ...
	 *            factory
	 * @param factoryClass
	 *            the class of the factory to instantiate and add
	 * @throws RuntimeException
	 *             if the given factory class cannot be instantiated
	 */
	...
	public Message create(String beginString, String msgType) {
		...
		if (message == null) {
			if (messageFactory != null) return messageFactory.create(beginString, msgType);
			message = new Message();
		}
	...
	public Group create(String beginString, String msgType, int correspondingFieldID) {
		MessageFactory messageFactory = messageFactories.get(beginString);
		if (messageFactory != null) return messageFactory.create(beginString, msgType, correspondingFieldID);
