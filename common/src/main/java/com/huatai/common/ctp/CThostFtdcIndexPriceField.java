package com.huatai.common.ctp;

import org.bridj.BridJ;
import org.bridj.Pointer;
import org.bridj.StructObject;
import org.bridj.ann.Array;
import org.bridj.ann.Field;
import org.bridj.ann.Library;
import org.bridj.ann.Name;
import org.bridj.cpp.CPPObject;

/**
 * \u80a1\u6307\u73b0\u8d27\u6307\u6570<br>
 * <i>native declaration : line 8951</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> or <a href="http://bridj.googlecode.com/">BridJ</a> .
 */
@Name("CThostFtdcIndexPriceField")
@Library("thosttraderapi")
public class CThostFtdcIndexPriceField extends StructObject {
	static {
		BridJ.register();
	}
	
	/**
	 * \u7ecf\u7eaa\u516c\u53f8\u4ee3\u7801<br>
	 * C type : TThostFtdcBrokerIDType
	 */
	@Array({ 11 })
	@Field(0)
	public Pointer<Byte> BrokerID() {
		return io.getPointerField(this, 0);
	}
	
	/**
	 * \u5408\u7ea6\u4ee3\u7801<br>
	 * C type : TThostFtdcInstrumentIDType
	 */
	@Array({ 31 })
	@Field(1)
	public Pointer<Byte> InstrumentID() {
		return io.getPointerField(this, 1);
	}
	
	/**
	 * \u6307\u6570\u73b0\u8d27\u6536\u76d8\u4ef7<br>
	 * C type : TThostFtdcPriceType
	 */
	@Field(2)
	public double ClosePrice() {
		return io.getDoubleField(this, 2);
	}
	
	/**
	 * \u6307\u6570\u73b0\u8d27\u6536\u76d8\u4ef7<br>
	 * C type : TThostFtdcPriceType
	 */
	@Field(2)
	public CThostFtdcIndexPriceField ClosePrice(double ClosePrice) {
		io.setDoubleField(this, 2, ClosePrice);
		return this;
	}
	
	public CThostFtdcIndexPriceField() {
		super();
	}
	
	public CThostFtdcIndexPriceField(Pointer<? extends CPPObject> pointer) {
		super(pointer);
	}
}
