 ...
 *
 */
public class OperatorUtilSuccForTest extends Operator4Redis {

	private static Logger logger = LoggerFactory.getLogger(OperatorUtilSuccForTest.class);

	private static OperatorUtilSuccForTest instance = new OperatorUtilSuccForTest();

	private final int DEFAULT_TIMEOUT = 3000;
	private final long DEFAULT_TRY = 200;
	private final int ALARM_MS = 100;

	private OperatorUtilSuccForTest() {
		;
	}
	 ...
	 */
	public static OperatorUtilSuccForTest getInstance() {
		return instance;
	}
	...
	public boolean set(String key, String value) {
		...
		while (true) {
			try {
				...
				jedisCluster.set(key, value);
				if (System.currentTimeMillis() - stime > ALARM_MS) {
					logger.error("opeartion too long : {}", key);
				}
				return true;
			} catch (Exception e) {
				logger.error("Operator Redis Error:" + e);
